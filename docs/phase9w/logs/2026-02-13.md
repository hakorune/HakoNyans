...
## 2026-02-13: `mode2` Counter/Prune Micro-Optimizations (No-Go, Reverted)

### Objective
- Reduce `compress_global_chain_lz` inner-loop overhead without changing output:
  1. local-accumulate counters (reduce per-step counter writes)
  2. prune candidates that cannot reach current `best_len`

### Trial Variants
- Variant A: local counter accumulation
  - moved hot-path counter increments to local vars and committed once at end.
  - CSV:
    - `bench_results/phase9w_mode2_localcounter_opt_vs_lostreamobs_20260213_runs3.csv`
    - `bench_results/phase9w_mode2_localcounter_opt_vs_lostreamobs_20260213_runs3_rerun.csv`
    - `bench_results/phase9w_singlecore_mode2_localcounter_opt_vs_afterfix_20260213_runs3.csv`
    - `bench_results/phase9w_singlecore_mode2_localcounter_opt_vs_afterfix_20260213_runs3_rerun.csv`
- Variant B: best-len prune
  - if candidate mismatches at `(best_len-1)` byte, skip full length extension.
  - CSV:
    - `bench_results/phase9w_mode2_bestlen_prune_vs_lostreamobs_20260213_runs3.csv`
    - `bench_results/phase9w_singlecore_mode2_bestlen_prune_vs_afterfix_20260213_runs3.csv`

### Validation
- Build: `cmake --build build -j`
- Tests: `ctest --test-dir build --output-on-failure`
- Result: `17/17 PASS` in all trials.
...
- Invariants preserved:
  ...
  - `median PNG/HKN = 0.2610`
- Speed signal:
  - no stable encode gain across reruns (multicore/single-core both mixed).
  - some runs showed stage-level movement, but end-to-end medians were not
    consistently better than baseline.

### Decision
- **No-Go (reverted)** for both variants.
- Keep current deterministic baseline and continue searching for larger mode2 wins.

## 2026-02-13: `mode2` Beat-Offset Prune Trial (No-Go, Reverted)

### Objective
- Reduce `match_len_from()` calls by pruning candidates that cannot beat
  current best match under tie-break rules.

### Implementation (trial-only)
- `src/codec/lossless_natural_route.h`
  - In chain loop, after 3-byte match:
    - if `best_len >= 3` and `dist >= best_dist`
    - and candidate mismatches at `best_len` offset
    - skip full length extension (`continue`)
...
### Benchmark Artifacts
- Multicore:
  - `bench_results/phase9w_mode2_beatoffset_prune_vs_lostreamobs_20260213_runs3.csv`
- Single-core:
  - `bench_results/phase9w_singlecore_mode2_beatoffset_prune_vs_afterfix_20260213_runs3.csv`
...
- Invariants preserved:
  ...
  - `median PNG/HKN = 0.2610`
- Speed:
  - `mode2` stage time and wall encode both regressed vs baseline in the trial runs.
  - single-core also regressed (`Enc median` worse than baseline).
